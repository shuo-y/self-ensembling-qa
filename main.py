...
parser.add_argument(
    ...
    type=str,
    default="",
    help='pass adaptive dataset path',
)
parser.add_argument(
    '--adap_rate',
    type=float,
    default=0.50,
    help='pass adaptive change length rate',
)
parser.add_argument(
    '--adap_type',
    type=str,
    default="Truncate",
    help='pass adaptive dataset type, default is trunk',
)
parser.add_argument(
    '--adap_rep',
    type=int,
    default=2,
    help='Pad how many unlabeled sample',
)
parser.add_argument(
    '--max_context_length',
    type=int,
...
def _calculate_loss_semi(args, start_logits, end_logits, start_positions, end_positions,
                         start_logits_teach, end_logits_teach):
    ignored_index = start_logits.size(1)
    assert len(start_positions) == len(end_positions)
    start_target = torch.ones(len(start_positions)).long()
    end_target = torch.ones(len(start_positions)).long()

    for i in range(len(start_positions)):
        if start_positions[i] == -1 or end_positions[i] == -1:
            start_target[i] = ignored_index
            end_target[i] = ignored_index
        else:
            start_target[i] = start_positions[i]
            end_target[i] = end_positions[i]

    start_target.clamp_(0, ignored_index)
    end_target.clamp_(0, ignored_index)

    # Compute the cross-entropy loss for the start and end logits.
    criterion = nn.CrossEntropyLoss(ignore_index=ignored_index)
    start_loss = criterion(start_logits, cuda(args, start_target))
    end_loss = criterion(end_logits, cuda(args, end_target))
    sup_loss = (start_loss + end_loss) / 2
    # Add thresholds in case the cross-entopy loss blows up
    sup_loss.clamp_(-10.0, 10.0)

    # Compute the MSE loss between the teacher and student network for self-ensembling.
    unsup_lossfunc = nn.MSELoss()
    # Add thresholds in case the MSE loss blows up
    start_loss_ = unsup_lossfunc(torch.clamp(start_logits, -10.0, 10.0), torch.clamp(start_logits_teach, -10.0, 10.0))
    end_loss_ = unsup_lossfunc(torch.clamp(end_logits, -10.0, 10.0), torch.clamp(end_logits_teach, -10.0, 10.0))

    unsup_loss = (start_loss_ + end_loss_) / 2

    return (sup_loss +  unsup_loss )


def train_with_teacher(args, epoch, model, ema_wrapper, dataset):
    """
    Trains the model using the self-ensembling method.
    ...
    Returns:
        Sum of the supervised cross entropy loss and 
        unsupervised MSE loss between teacher and student network
    """
        # Set the model in "train" mode.
    model.train()
    ...
    for batch in train_dataloader:
        ...
        # Forward inputs, calculate loss, optimize model.
        start_logits, end_logits = model(batch)
        start_logits_teach, end_logits_teach = ema_wrapper.teacher_model(batch)

        loss = _calculate_loss_semi(
            args,
            start_logits,
            end_logits,
            batch['start_positions'],
...
def main(args):
    ...
    # Set up datasets.
    if args.model == "dev":
        assert(len(args.adap_path) > 0)
        train_dataset = QADataset(args, args.train_path, args.adap_path)
    else:
    ...
    if args.do_train:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if args.model == "baseline":
                ...
                train_loss = train_with_teacher(args, epoch, model, ema_wrapper, train_dataset)
