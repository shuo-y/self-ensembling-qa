...
class QADataset(Dataset):
    ...
    """
    def __init__(self, args, path, adap_path):
        self.args = args
        self.meta, self.elems = load_dataset(path)
        self.samples = self._create_samples(self.elems)

        if len(adap_path) > 0:
            # Append for domain adaptation
            self.adap_meta, self.adap_elems = load_dataset(adap_path)
            adap_samples = self._create_samples(self.adap_elems, adapt=True)
            self.samples.extend(adap_samples)
    ...
    def proc_passage(self, args, original_passage):
        """
        Data augmentation. Generate random unlabeled passage
        based on the original_passage

        Return:
            A new passage which is a random subset of the original passage
            if the args.adap_type is Truncate. Or a new passage which is
            the original passage extened with a random subset if the args.adap_type
            is Extend.
            If args.adap_type is not specificed. Return a passage which is
            the same as the original_passage.
        """
        lens = len(original_passage)
        lens = int(lens * args.adap_rate)
        if args.adap_type == "Truncate":
            start_idx = randint(0, lens)
            new_passage = original_passage[(start_idx):(start_idx + lens)]
            return new_passage
        elif args.adap_type == "Extend":
            start_idx = randint(0, lens)
            append_passage = original_passage[(start_idx):(start_idx + lens)]
            new_passage = original_passage + append_passage
            return new_passage
        return original_passage


    def _create_samples(self, elems, adapt=False):
        """
        ...
        for elem in elems:
            ...
            for qa in elem['qas']:
                ...
                if adapt:
                    answer_start = -1
                    answer_end = -1
                    for tt in range(self.args.adap_rep):
                        new_passage = self.proc_passage(self.args, passage)
                        samples.append(
                            (qid, new_passage, question, answer_start, answer_end)
                        )
                else:
                    samples.append(
