...
class BaselineReader(nn.Module):
    ...
    def forward(self, batch):
        self.passage_rnn.flatten_parameters()
        self.question_rnn.flatten_parameters()
...
class EMA(nn.Module):
    ...
    # Compute the exponential moving average of studeng network weights
    # to update the weight of teacher network
    def update(self):
        teacher_params = OrderedDict(self.teacher_model.named_parameters())
        student_params = OrderedDict(self.student_model.named_parameters())
        assert teacher_params.keys() == student_params.keys()

        for name, param in student_params.items():
            teacher_params[name].sub_((1. - self.decay) * (teacher_params[name] - param))
